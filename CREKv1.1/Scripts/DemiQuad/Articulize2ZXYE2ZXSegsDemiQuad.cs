...
public class Articulize2ZXYE2ZXSegsDemiQuad : MonoBehaviour
{
    ...
    public void assignPhysScripts()
    {
        ...
        landmarks["ZXYK16Axis"].gameObject.AddComponent<ArticulatedZXYESegsQuad>();

        landmarks["LeftFrontZ"].gameObject.AddComponent<ArticulatedZXYE>();
        landmarks["RightFrontZ"].gameObject.AddComponent<ArticulatedZXYE>();
        landmarks["LeftRearZ"].gameObject.AddComponent<ArticulatedZXYK>();
        landmarks["RightRearZ"].gameObject.AddComponent<ArticulatedZXYK>();


        ArticulatedZXYESegsQuad parent = landmarks["ZXYK16Axis"].gameObject.GetComponent<ArticulatedZXYESegsQuad>();

        ArticulatedZXYE leftfront = landmarks["LeftFrontZ"].gameObject.GetComponent<ArticulatedZXYE>();
        ArticulatedZXYE rightfront = landmarks["RightFrontZ"].gameObject.GetComponent<ArticulatedZXYE>();
    ...
    void setArticulationDrive()
    {
        ...
        lrzad.stiffness = lrxad.stiffness = springValue;
        rrzad.stiffness = rrxad.stiffness = springValue;


        lfzad.damping = lfxad.damping = lfead.damping = damperValue;
        rfzad.damping = rfxad.damping = rfead.damping = damperValue;
        lrzad.damping = lrxad.damping = damperValue;
        rrzad.damping = rrxad.damping = damperValue;
    ...
    void configureArticulationBodies()
    {
        ...
        //rear 4 axis
        lrzab.jointType = lrxab.jointType = ArticulationJointType.RevoluteJoint;
        rrzab.jointType = rrxab.jointType = ArticulationJointType.RevoluteJoint;
