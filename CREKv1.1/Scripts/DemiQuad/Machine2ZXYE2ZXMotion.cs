...
public class Machine2ZXYE2ZXMotion : MonoBehaviour
{
	public CRTaskDBController nsdb;
	...
	public void tick(float userInput)
	{
		if (!isActive)
			return;
	...
	Trajectory resolveTrajByNodeName(string nodeName)
	{
		...
		else if (trajType == "CIRCLEDIRTRAJECTORY")
		{
			//this is a generated traj node
			//get the inputnodes from this trajectory node
			//and resolve, then use them to create trajectory points generateTrajectory(a,b);
			...
			Debug.Log("aa,  rr: " + aa.ToString() + ", " + rr.ToString());
			Debug.Log(nodeInputString);
			...
			//direction vectors are normalized, with a length of 1,
			//we want a radius that's about 3 times as what we expect
			planner.generatePointCircle(0.4f, ref points);

			trajJSON.keyframes.Add(new KeyFrameJSON("raw", aa));
			for (int i = 0; i < points.Length; i++)
			{
				points[i] = Geometry.rotateAroundYAxis(Mathf.PI / 2f, points[i]); //turn it 90 deg about Y
				points[i] = Geometry.rotateAroundZAxis(Mathf.PI / 2f, points[i]); //turn it 90 deg about Z

				trajJSON.keyframes.Add(new KeyFrameJSON("raw", (aa + points[i])));
				Debug.Log((aa + points[i]).ToString("F3"));
