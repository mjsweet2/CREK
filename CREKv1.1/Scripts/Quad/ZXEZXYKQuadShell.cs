...
public class ZXEZXYKQuadShell : MonoBehaviour
{
    ...
    public CRTaskDBController db;
    public ArticulatedZXYESegsQuad articulated2ZXE2ZXYK;
    ...
    public void runCmd(string inputCmd)
    {
        ...
        if (iTokens.Length > 0)
        {
            ...
            else if ((firstNodeNamelf + firstNodeNamelr + firstNodeNamerf + firstNodeNamerr + firstNodeNamesegs) != "")// this is a motion call
            {
                ...
                if (paramsLoaded)
                {
                    if (machine2ZXE2ZXYK.motionMode == Machine4ZXYESegs.MOTIONMODE.MOTION)
                    {
                        machine2ZXE2ZXYK.runMotionOnAllChannels(iTokens[0]);
                    }
                    else if (machine2ZXE2ZXYK.motionMode == Machine4ZXYESegs.MOTIONMODE.TRAJECTORY)
                    {
