...
public class ComponentZXYK : MonoBehaviour
{
    ...
    public enum IKMODE { ZXK, ZXYK };
    public IKMODE ikMode;

    public string compName; // self.name = "base_class"     # for logging
    public Vector4 lastValidIk;   // The Component is storing state, so I need a seperate component for each
    public Vector3 lastValidFk;   // self.last_valid_fk =  [0.0,0.0,0.0]
    public Vector4 inputIk;     // used for fk calcs, not used for ik calcs,
    ...
    {
        if (side == SIDE.LEFT)
            doLeftIk(targetFk.position.x, targetFk.position.y, targetFk.position.z);
        else
    ...
    public Geometry.GeoPacket doRightIk(float x, float y, float z)
    {
        ...
        //add result to upper+middle bone in dormant position
        intermediateEndEffector += new Vector3(0f, -(upper + middle), 0f);

        if (iA != null)
            iA.position = intermediateEndEffector;
        ...
        if (iB != null)
            iB.position = intermediateEndEffector;
        ...
        //take 1
        {
            ...
            float zRads = Geometry.angleBetweenVectors(zTempU, zTempV);

            //angle between vectors doesn't tell us direction, so the easiest thing
            //is to try both directions and test which is closer to our requesteds
            Vector3 zOneWay = Geometry.rotateAroundZAxis(zRads, intermediateEndEffector);
            Vector3 zOtherWay = Geometry.rotateAroundZAxis(-zRads, intermediateEndEffector);
            ...
            ik.x = zRads / (2 * Mathf.PI);

            */
        }
        //can I calc this much easier?
        //take 2
        {
            float zRads = Mathf.Atan(requested.x / requested.y);
