...
public class ZXEZXYKQuadShell : MonoBehaviour
{
    ...
    public NSMotionDBController db;
    ...
    bool loadMotionInputs(ref string[] tokens)
    {
        ...
        if (retStringrf == "") retStringrf = JsonUtility.ToJson(new NSInputsJSON());
        if (retStringrr == "") retStringrr = JsonUtility.ToJson(new NSInputsJSON());
        ...
        NSInputsJSON rfJSON = JsonUtility.FromJson<NSInputsJSON>(retStringrf);
        NSInputsJSON rrJSON = JsonUtility.FromJson<NSInputsJSON>(retStringrr);
